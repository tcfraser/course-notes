...
\subsubsection{Implications of $\ba{n_\vep}$}
\textbf{Fermions}:\\
...
\textbf{Bosons}:\\
There is no restriction for Bosons to occupy all the same $\vp \approx 0$ momentum state. What is remarkable is the evolution from ``normal gas'' to one with $n \de \br{\vp} = n\br{T} \de \br{\vp}$ appears/proceeds through a \textit{phase transition} called \textbf{Bose-Einstein condensate (BEC)}. This was predicted in 1924-1925 by Satyendra, Bose and Einstein and was not observed until June 5th, 1995 where is was found that,
