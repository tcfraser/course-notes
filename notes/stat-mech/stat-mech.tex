...
\subsection{Polarizable Molecule}
...
\begin{tikzpicture}
        ...
        \coordinate (nxa) at (0, 0);
        \coordinate (pxa) at (5, 0);
        \node [above] (lpya) at (pya) {$C$};
        \node [right] (lpxa) at (pxa) {$\f{k_BT}{\vep}$};
        \draw [->, thick] (nya) -- (pya);
        \draw [->, thick] (nxa) -- (pxa);
        \draw[domain=0.01:5,smooth,thick, variable=\x,red]  plot ({\x},{5*(1)/(\x^2) * (e^(-1/\x))/(1+e^(-1/\x))^2});
\end{tikzpicture}
\end{center}
