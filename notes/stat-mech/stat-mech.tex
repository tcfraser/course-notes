...
But does labeling the particles really matter? For the case of $N \to \infty$, the global phenomenology is of interest.
...
\[ C \defined \lim_{\Delta T \to 0} \f{\Delta Q}{\Delta T} \]
...
\[ c_v = \lim_{\dif T \to 0} \f{\indif q}{\dif T} \note{At constant V} \]
...
\[ \lim_{\dif T \to 0} \bre{\f{\indif q}{\dif T}}_{p=\text{const.}} = c_v + R \]
...
Which gives that $\De Q / T$ over the whole cycle is zero. Taking the limit as $k \to \inf$, it suggests that the quantity $\indif Q / T$ is an exact differential. Introduce the label of entropy,
...
\[ \sum_{i=1}^{n} \f{\indif Q'_i}{T_i} < 0 \to \ointl \f{\indif Q'}{T} < 0  \]

This leads to the Clausius Inequality:

\[ \ointl \f{\indif Q'}{T} \leq 0 \]

Which achieves equality only when the \textbf{entire} cycle is reversible. Inequality is always maintained if \text{any} part of the cycle is irreversible. This can be understood as another formulation of the Second Law of Thermodynamics. \\
...
\begin{enumerate}
        \item Systems where $V \to \inf$ and $N \to \inf$ and the density $N/V$ remains constant
        \begin{itemize}
...
Now in the limit that $\De/E \to 0$,

\[ \lim_{\De/E \to 0} \Ga_{N, V, \bar{E} = E} = \Ga_{N, V \bar{E} = E} = \lim_{\De/E \to 0} \bs{{\sum}_N \br{N, V, E + \f{\De}{2}} - {\sum}_N \br{N, V, E - \f{\De}{2}}} = \pder{{\sum}_N}{E} \cdot \De \]
...
Similarly for $\vec{p}_\mu$
\[ \vec{p}_\mu = p_{\al,\mu} \mapsto p_i \quad \al = 1,2,3 \quad  \mu = 1,2,\ldots, N \quad  i = 1,2,\ldots,3N \]
...
\[ \vr_\mu , \vec{p}_\mu \mapsto \br{r_x, r_y, r_z, p_x, p_y, p_z} \]
...
Where $i$ counts the degrees of freedom. We can then perform a change of variables $p \to y$ to get,
...
\[ \al \to \sum_r \de n_r = 0 \qquad \be \to \sum_r \de n_r \cdot E_r= 0 \]
...
More explicitly,
\[ T = \sum_{i=1}^{N} \f{\abs{\vec{p}_i}^2}{2m} \qquad V = \sum_{i > j} V_{ij} \br{q_{ij}}\]
...
\[ Z = \int_{\w} e^{-\be\bc{\sum_{i=1}^{N} \f{\abs{\vec{p}_i}^2}{2m}} - \be \bc{\sum_{i > j} V_{ij} \br{q_{i},q_{j}}}} \f{\dif^{3N} q \dif^{3N} p}{h^{3N} N!} \]
...
\[ Z = \int_{\w_p} e^{-\be\bc{\sum_{i=1}^{N} \f{\abs{\vec{p}_i}^2}{2m}}} \f{\dif^{3N} p}{h^{3N} N!} \times \int_{\w_q} e^{- \be V \br{q}} \dif^{3N} q \]
...
\[ \int_{\w_p} e^{-\be\bc{\sum_{i=1}^{N} \f{\abs{\vec{p}_i}^2}{2m}}} \f{\dif^{3N} p}{h^{3N} N!} = \int_{\w_p} e^{-\be\br{\f{p_{1,x}^2}{2m} + \cdots + \f{p_{2,y}^2}{2m} + \cdots}} \f{\dif^{3N} p}{h^{3N} N!} = \bs{\intl_{-\inf}^{\inf} e^{-\be\f{p_{\al}^2}{2m}} \dif p_\al}^{3N} \f{1}{h^{3N} N!} \]
...
\[ H = \sum_{i=1}^N \f{\abs{\vec{p}_i}^2}{2m} = \f{p_{1,x}^2 + p_{1,y}^2 + p_{1,z}^2}{2m} + \cdots + \f{p_{N,x}^2 + p_{N,y}^2 + p_{N,z}^2}{2m} \]
Each $\vec{p}_i$ represents the total momentum vector of the particle. Be careful of this distinction. It should be evident from the context of the problem. \\
...
\[ H_i \br{p,q} = \f{\abs{\vec{p}}^2}{2m} - \vec{\mu} \cdot \vE \]
...
\[ H_i \br{p,q} = \f{\abs{\vec{p}}^2}{2m} - \mu E \cos \te \]
...
\[ H = \f{\abs{\vec{p}}^2}{2m} - \mu E \cos \te + H\tsb{rot} \]
...
But $C_V \propto N$ is an extensive variable. Also $U \propto N$ and $T$ doesn't scale with temperature. For very large $N$, the root-mean-squared fluctuations in energy go as $1/\sqrt{N}$ and thus are negligible for large $N$. In this limit, the varying energy becomes ultimately zero. Therefore for $N \to \inf$, the canonical ensemble acts like the microcanonical ensemble. \\
...
The Equipartition theorem is really useful to estimate or determine thermodynamic properties of systems by analyzing the degrees of freedom of the system. Consider a Hamiltonian $H \br{\vq, \vec{p}}$. Let's call $x_i$ and $x_j$ any two of the $6N$ generalized coordinates of the system $\br{q,p}$. In the canonical ensemble we are interested in the expectation,
\[ \ba{x_i \pder{H}{x_j}} \]
Explicitly,
...
Then adding $i=1\to3N$ for both $p_i$ and $q_i$,
...
\subsection{Physical Significance of \texorpdfstring{$\al, \be$}{Lg}}
...
\[ \eta = \ln \br{\sum_{i,j} e^{-\al N_i - \be N_j}} = \f{PV}{k_B T} \eq \label{eq:eta2}\]
...
Clearly $\eta$ given by \eqref{eq:eta2} plays a crucial role. It can be rewritten as,
...
\[ \sum_{i} e^{\be \mu N_i} \to \sum_{N_i = 0}^{\inf} e^{\be \mu N_i} \]
...
\[ \sum_{k_m} \to \int \f{\dif k_m}{L /2 \pi} \]
...
\textbf{Example}:
For spin systems, there might be adjusts to the formulas above. Generally, there is a ``internal structure''/ multiplicative factor.
\[ \bar{E} = \sum_{\vep} \f{g_s \vep}{e^{\be \br{\vep - \mu}} + a} = g_s \br{\f{L}{2\pi}}^3 4 \pi \intl_{0}^{\inf} \f{\br{\f{\hbar^2 k^2}{2m}} k^2 \dif k}{e^{\be\br{\f{\hbar^2 k^2}{2m} - \mu}} + a} \]
\[ \bar{N} = \sum_{\vep} \f{g_s}{e^{\be \br{\vep - \mu}} + a} = g_s \br{\f{L}{2\pi}}^3 4 \pi \intl_{0}^{\inf} \f{k^2 \dif k}{e^{\be\br{\f{\hbar^2 k^2}{2m} - \mu}} + a} \]
In practice, one needs to invert these integrals to obtain $\mu$ for any given system.

\subsubsection{Implications of \texorpdfstring{$\ba{n_\vep}$}{Lg}}
\textbf{Fermions}:\\
Because the only electrons/fermions capable of being reshuffled by thermal fluctuations are closed to the Fermi energy, one has that all the properties of the metal are controlled by the that \textit{sliver} of electrons for $\vep \approx \vep_F \pm \De$ where $\De = k_B T$. For real/ordinary metals $T_F \sim 10^4- 10^5 \text{K}$. As such $T/T_F \ll 1$. Electrons in metals are essentially/almost at $T = 0^+$. \\

Because fermions obey the exclusion principle, they exert pressure (called degeneracy pressure). In a white dwarf, it is the Fermi pressure that holds the gravity of the star. There is a limit (the Chandrasekhar limit $M\tsb{CH}$) when the mass of the white dwarf becomes more massive that $M\tsb{CH}$ where white dwarfs collapse into neutron stars and then beyond that, black holes (quark stars, superconducting cores/color superconductivity). \\

\textbf{Bosons}:\\
There is no restriction for Bosons to occupy all the same $\vec{p} \approx 0$ momentum state. What is remarkable is the evolution from ``normal gas'' to one with $n \de \br{\vec{p}} = n\br{T} \de \br{\vec{p}}$ appears/proceeds through a \textit{phase transition} called \textbf{Bose-Einstein condensate (BEC)}. This was predicted in 1924-1925 by Satyendra, Bose and Einstein and was not observed until June 5th, 1995 where is was found that,
\[ n \lambda_T^3 \sim 1 \quad \lambda_T = \f{h}{\br{2 \pi m k_B T}^{1/2}} \quad T_c \sim 3.3125 \f{\hbar^2 n^{2/3}}{m k_B} \]
Below this critical temperature, all of the particles merge into a single wave function. Superfluid $^4\text{He}$ has $T_c \sim \SI{2.17}{\K}$ is a complex form of a BEC. Also $^3\text{He}$ (a fermion) is also a superfluid with $T \sim \SI{1}{\micro \K}$. This is achieved when two fermions form a \textbf{Cooper pair} to form a composite boson that can then be thought of as a BEC. Superconductivity is a form of a BEC.
\end{document}
