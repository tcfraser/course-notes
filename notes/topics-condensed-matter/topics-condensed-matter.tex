...
\[ M = \f12 \tanh \bs{\f{\s J z M}{2 k_B T}} \eq \label{eq:magnetization_M}\]
\begin{center}
    \begin{tikzpicture}
    ...
    \end{tikzpicture}
\end{center}
Evidently there are at most $3$ solutions depending on the temperature. There are three solutions if the slope of $\tanh \f{\s J z M}{2 k_B T}$ is greater than $1$ at $M = 0$. Using a Taylor series,
\[ \tanh x \simeq x \]
The slope of $\f12\tanh \br{\f{\s J z M }{2 k_B T}}$ near $M=0$ is simply,
\[ \f{\s J z}{4 k_B T} \]
This defines a \term{critical temperature},
\[ \f{\s J z}{4 k_B T} > 1 \implies \f{\s J z}{4 k_B T_c} = 1 \implies T_c = \f{\s J z}{4 k_B} \]
Which distinguishes two different phases,
\begin{itemize}
...
\[ H\tsb{int} = U \sum_{i} n_{i \uparrow}n_{i \downarrow} \eq\label{eq:hubbard_model_int}\]
with $n_{i, \si}$ denotes the number operator for the number of atoms localized to atom $i$ with spin $\si$. Terms of the form $n_{i \uparrow}n_{i \uparrow}$ are not present due to the Pauli exclusion principle. The Hubbard model is simply \cref{eq:hubbard_model_kin} combined with \cref{eq:hubbard_model_int},
\[ H = - t \sum_{\ba{ij}\si} \br{\ket{i \si}\bra{j\si} + \ket{j \si}\bra{i \si}} + U \sum_{i} n_{i \uparrow}n_{i \downarrow} \eq \label{eq:hubbard_ham} \]
...
Therefore,
\[ n_{i \uparrow}n_{i\downarrow} = \br{\ba{n_{i\uparrow}} + n_{i\uparrow} - \ba{n_{i\uparrow}}} \br{\ba{n_{i\downarrow}} + n_{i\downarrow} - \ba{n_{i\downarrow}}} \]
\[ n_{i \uparrow}n_{i\downarrow} = \br{n_{i\uparrow} - \ba{n_{i\uparrow}}}\br{n_{i\downarrow} - \ba{n_{i\downarrow}}} + n_{i \uparrow} \ba{n_{i \downarrow}} + n_{i \downarrow}\ba{n_{i \uparrow}} + \ba{n_{i \uparrow}}\ba{n_{i \downarrow}} - 2\ba{n_{i \uparrow}}\ba{n_{i \downarrow}} \]
Once again we assume that the fluctuations are small,
\[ n_{i \uparrow}n_{i\downarrow} = n_{i \uparrow} \ba{n_{i \downarrow}} + n_{i \downarrow}\ba{n_{i \uparrow}} - \ba{n_{i \uparrow}}\ba{n_{i \downarrow}} \]
Now we let $n$ denote the average number of electrons per atom (here we assume that this number is uniform $n_i = n$),
\[ n = \ba{n_{i \uparrow}} + \ba{n_{i \downarrow}} \]
We define the \term{degree of spin polarization} $\xi$ such that,
\[ \xi \defined \f{\ba{n_{i \uparrow}} - \ba{n_{i \downarrow}}}{\ba{n_{i \uparrow}} + \ba{n_{i \downarrow}}} \]
\[ \ba{n_{i\uparrow}} = n \f{1 + \xi}{2} \qquad \ba{n_{i\downarrow}} = n \f{1 - \xi}{2} \]
Where $\xi_i = \xi$ is uniform as well for simplicity. Now we have \cref{eq:hubbard_ham} taking the following form,
\[ H = -t \sum_{\ba{ij}\si}\br{\ket{i \si}\bra{j \si} + \ket{j \si}\bra{i \si}} + \f{Un}{2} \sum_{i} \br{n_{i \uparrow} + n_{i \downarrow}} - \f{Un\xi}{2} \sum_{i} \br{n_{i \uparrow} - n_{i \downarrow}} - \f{Un^2}{4} \br{1 - \xi^2} N \]

\subsection{Mott Insulators and Anti-Ferromagnetism}
\todo[TC]{Missed two lectures}
The ground state wavefunction was found to be,
\[ \ket{\psi} = \f{1}{\sqrt{2}} \br{\ket{1\uparrow, 2 \downarrow} - \ket{1 \downarrow, 2 \uparrow}} \]
Where $\ket{\psi}$ corresponded to a total spin of $\ve S = 0$ and a ground state energy of,
\[ E_g = - \f{4t^2}{U} \]
Where $E_g$ is obtained via the following,
\[ H \ket{\psi} = \s J \ve S_1 \cdot \ve S_2 \ket{\psi} = E_g \ket{\psi} \]
...
\[ \ket{\psi} \in \bc{\ket{\uparrow, \uparrow}, \ket{\downarrow, \downarrow}} \]
...
\begin{align*}
    S^z \ket{\uparrow} &= \f{\hbar}{2}\si^{z}\ket{\uparrow} = \f{\hbar}{2}\ket{\uparrow} \\
    S^z \ket{\downarrow} &= \f{\hbar}{2}\si^{z}\ket{\downarrow} = \f{\hbar}{2}\ket{\downarrow} \\
    S^+ \ket{\uparrow} &= \f{\hbar}{2}\si^{+}\ket{\uparrow} = \f{\hbar}{2} \begin{pmatrix}
        0 & 2 \\
        0 & 0 \\
    ...
    \end{pmatrix}\begin{pmatrix}
        1 \\ 0
    \end{pmatrix} = \hbar \ket{\downarrow} \
    S^+ \ket{\downarrow} &= \f{\hbar}{2}\si^{+}\ket{\downarrow} = \f{\hbar}{2} \begin{pmatrix}
        0 & 2 \\
        0 & 0 \\
    \end{pmatrix}\begin{pmatrix}
        0 \\ 1
    \end{pmatrix}  = \hbar \ket{\uparrow} \\
    S^- \ket{\downarrow} &= \f{\hbar}{2}\si^{+}\ket{\downarrow} = \f{\hbar}{2} \begin{pmatrix}
        0 & 0 \\
        2 & 0 \\
    \end{pmatrix}\begin{pmatrix}
    ...
    \end{pmatrix} = 0
\end{align*}
Therefore $\ket{\uparrow, \uparrow}$ is an eigenstate of $H$ with eigenenergy $- \s J \hbar^2 / 4$,
\[ H \ket{\psi} = -\f{\s J \hbar^2}{4}\bs{\si_1^{z}\si_2^{z} + \f12 \br{\si_1^+ \si_2^- + \si_1^- \si_2^+}} \ket{\uparrow, \uparrow} = - \f{\s J \hbar^2}{4}\ket{\uparrow, \uparrow} \]
...
\[ H = + \s J \ve S_1 \cdot \ve S_2 \]
A simple guess for the ground state would reasonably be $\ket{\uparrow, \downarrow}$ (or $\ket{\downarrow, \uparrow}$). This however does not work,
\[ H \ket{\psi} = \f{\s J \hbar^2}{4}\bs{\si_1^{z}\si_2^{z} + \f12 \br{\si_1^+ \si_2^- + \si_1^- \si_2^+}} \ket{\uparrow, \downarrow} \not \propto \ket{\uparrow, \downarrow} \]
Therefore $\ket{\uparrow, \downarrow}$ is \textit{not} the ground state of for anti-ferromagnets. This state, and the analogous multi-spin state, where neighboring spin states have opposite spin, is called the \term{Neel state}.
\begin{center}
    \begin{tikzpicture}
...
Superconductivity is the phenomena in materials where resistivity suddenly drops to zero below a critical temperature $T_c$,
...
A related feature is that that specific heat of the material jumps at the critical temperature as well.
