...
\begin{center}
    \begin{tikzpicture}
        ...
        \draw[thick, scale=1.0,domain=-1.5:1.5,smooth,variable=\M,red   ] plot ({\M},{2*\M}) node[above]{$M$};
...
\[ \tanh x \simeq x \]
Therefore the slope of $\tanh \br{\f{\s J z M }{2 k_B T}}$ near $M=0$ is simply,
\[ \f{\s J z}{2 k_B T} \]
Therefore we define the critical temperature,
\[ \f{\s J z}{2 k_B T} > 1 \implies \f{\s J z}{2 k_B T_c} = 1 \implies T_c = \f{\s J z}{2 k_B} \]
