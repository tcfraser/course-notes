...
\begin{align*}
\bramidket{1,M}{V}{1,M}
&= \iint \dif^3 r_1 \dif^3 r_2 \f{1}{\sqrt{2}}\br{\varphi^*_1\br{\ve r_1} \varphi^*_2\br{\ve r_2} - \varphi^*_2\br{\ve r_1} \varphi^*_1\br{\ve r_2}} V\br{\ve r_1 - \ve r_2} \f{1}{\sqrt{2}}\br{\varphi_1\br{\ve r_1} \varphi_2\br{\ve r_2} - \varphi_2\br{\ve r_1} \varphi_1\br{\ve r_2}}\\
&= \f{1}{2}\iint \dif^3 r_1 \dif^3 r_2 V\br{\ve r_1 - \ve r_2} \bigg[\abs{\varphi_1\br{\ve r_1}}^2 \abs{\varphi_2\br{\ve r_2}}^2 + \abs{\varphi_1\br{\ve r_2}}^2 \abs{\varphi_2\br{\ve r_1}}^2  + \cdots\\
...
\begin{align*}
\bramidket{1,M}{V}{1,M}
&= \iint \dif^3 r_1 \dif^3 r_2 V\br{\ve r_1 - \ve r_2} \bigg[\abs{\varphi_1\br{\ve r_1}}^2 \abs{\varphi_2\br{\ve r_2}}^2 - \varphi_1^{*}\br{\ve r_1} \varphi_2^{*}\br{\ve r_2}\varphi_2\br{\ve r_1} \varphi_1\br{\ve r_2}\bigg]
\end{align*}
...
\[ \s C = \iint \dif^3 r_1 \dif^3 r_2 V\br{\ve r_1 - \ve r_2} \bs{\abs{\varphi_1\br{\ve r_1}}^2 \abs{\varphi_2\br{\ve r_2}}^2} \]
...
\[ \s J = \iint \dif^3 r_1 \dif^3 r_2 V\br{\ve r_1 - \ve r_2} \bs{\varphi_1^{*}\br{\ve r_1} \varphi_2^{*}\br{\ve r_2}\varphi_2\br{\ve r_1} \varphi_1\br{\ve r_2}} \]
In conclusion,
...
Pairs of electrons form bosonic states and are considered bosons. Bosons are not constrained by the Pauli exclusion principle; any number of bosons may occupy the ground state. Therefore at zero or low temperatures, all the bosons will want to \textit{condense} into the lowest energy state; the state with zero momentum. In this state, all of the bosons are essentially described by a single wavefunction $\varphi\br{\ve r}$. Landau and Ginzburg realized that one may think of $\varphi\br{\ve r}$ as an order parameter, just like magnetization in a ferromagnet. Near $T_c$ we may then write down a Taylor expansion of the free energy $F$ in powers of $\varphi$. Since the free energy should not depend on the phase of $\varphi\br{\ve r}$, it may only depend on $\abs{\varphi}^2, \abs{\varphi}^4, \ldots$ and so on. Thus,
\[ F = F_0 + V a \abs{\varphi}^2 + \f12 V b \abs{\varphi}^4 + \cdots \]
...
\[ \f{1}{V} \pder{F}{\abs{\varphi}} = 2 a \abs{\varphi} + 2 b \abs{\varphi}^3 = 0 \]
Therefore we have a ground state,
\[ \abs{\varphi} = \sqrt{- \f{a}{b}} \]
Which is possible when $a < 0$ ($b > 0$ always) when $T < T_c$. Therefore,
\[ a \sim \f{T - T_c}{T_c} \]
Therefore,
\[ \abs{\varphi} \sim \sqrt{T_c - T} \sim \br{-t}^{\be} \qquad \be = \f12 \]
To describe properties of superconductors it turns out to be very important to describe situations when $\varphi\br{\ve r}$ is not uniform. In this case we need to add extra terms to the free energy which depend on the non-uniformity of $\varphi\br{\ve r}$; specifically $\vdel \varphi\br{\ve r}$. Naively, the following should work by symmetry,
\[ F = \kappa \intl \dif^3 r \abs{\vdel \varphi}^2 \]
...
In our case, $\varphi\br{\ve r}$ is a wave-function of particles with charge $-2e$ (one for each electron). Thus the free energy must be invariant under the following,
\[ \varphi \mapsto e^{i \s X} \varphi \qquad \ve A \mapsto \ve A - \f{\hbar c}{2e} \vdel \s X \]
This means that the free energy must have the following form,
\[ F = \int \dif^3 r \bs{\f{1}{2 m^*} \abs{\br{-i \hbar \vdel  + \f{e^{*}}{c} \ve A}\varphi}^2 + a \abs{\varphi}^2 + \f{b}{2}\abs{\varphi}^4 + \f{\abs{\ve B}^2}{8\pi} } \]
Where $e^* = 2e$ and $m^* = 2m$.\\

\todo{Missed a week of lectures}
...
\[ I = I_c \sin \te + \f{V}{R} \]
Therefore given that the external potential is related to the time-derivative of the phase,
\[ V = \f{\hbar}{2e} \pder{\te}{t} \]
We conclude that,
...
Letting $\la = I_c/I < 1$ and $\w_0 = \f{2IeR}{\hbar}$ we obtain the following non-linear differential equation,
