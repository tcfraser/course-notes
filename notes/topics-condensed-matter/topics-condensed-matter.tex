...
\[ \nu = \left\lfloor{\f{N}{N_{\phi}}}\right\rfloor \qquad \nu = 0, 1, 2,\ldots \eq \label{eq:nu}\]
...
\[ \mu\br{B} = \hbar \f{e B}{mc} \br{\left\lfloor{\f{N{2 \pi \hbar c}}{{L_xL_y eB}}}\right\rfloor + \f32} \]
...
In particular, \term{ferro-magnetism} is the appearance of \textit{spontaneous} macroscopic magnetic moment in certain materials at sufficiently low temperature $T$. For a material magnetic field $H$, the magnetization $M$ is
...
\[ \ket{i \si} = \sum_{k} \ket{k \si} \braket{k \si}{i \si} = \f{1}{\sqrt{N}} \sum_{k} \ket{k \si} e^{- i \ve k \cdot \ve R_i} \]
...
\[ \ve S_1 \cdot \ve S_2 = S_1^xS_2^x + S_1^yS_2^y + S_1^zS_2^z = \f12 \br{S_1^+ S_2^- + S_1^- S_2^+} + S_1^z S_2^z \]
...
Where $\xi$ is termed the \term{coherence length} and measures how quickly the superconducting state transitions across the boundary $x=0$,
