...
Where ${\Ga^\ga}_{\al\be}$ is the coefficients of the \textit{connection} in the basis $e_\al$. The \textit{Christoffel symbols} are an example. Therefore what is the expression for $\del_X Y$? Writing $X, Y$ as their basis expansions,
...
\[ \del_X Y = X^\al\del_{e_\al}\br{Y^\be e_\be} \]
Now using \eqref{eq:linearitycov2},
\begin{align*}
...
&= X\br{Y^\be} e_\be + Y^\be \del_X e_\be \\
\end{align*}
...
% \[ \del_X Y = X{Y^\be} X^\al\del_{e_\al}\br{ e_\be} \]
% Where acting on the particular set of components of $Y$, $\del_\al Y^\be$ becomes a tensor of rank $\br{1,1}$.
% \[  \]
\end{document}
