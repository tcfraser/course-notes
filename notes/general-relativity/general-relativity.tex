...
Therefore homogeneity and isotropy immediately imply that we have a maximally symmetric space. One example is hyperplane $\vR^3$ with zero curvature $k = 0$. A second example is $S^3$ the 3-sphere with curvature $k = 1$ and finally the Hyperboloid with curvature $k=-1$. \\
\begin{center}
\begin{tabular}{|c|c|c|}
    \hline
    Space & Curvature $k$ \\
...
\end{tabular}
\end{center}
In order to obtain the spatial metric, we can solve \eqref{eq:maxsym}. Beginning with the most general homogeneous, isotropic metric,
...
\[ \ga_{ij} \dif x^i \dif x^j = \f{1}{1 - k r^2} \dif r^2 + r^2 \dif \Om^2 \]
