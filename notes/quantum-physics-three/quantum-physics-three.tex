...
This process can be continued to find the $n$-th order correction to $U\br{t}$. The resultant series is terms the \term{Dyson series},
\[ U^{(I)}\br{t} = \ident - \f i \hbar \intl_{0}^{t} \dif t' V\br{t'} + \cdots + \br{\f i \hbar}^n \intl_{0}^{t} \dif t' \intl_{0}^{t'} \dif t'' \cdots \intl_{0}^{t^{(n-1)}} \dif t^{(n)} V\br{t'} \cdots V\br{t^{(n)}} + \cdots \]
