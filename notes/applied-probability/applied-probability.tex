...
\begin{theorem}
    ...
    \[ \Var \br{Y} = \Exp\bc{\Var\br{Y \mid X}} + \Var\bc{\Exp\br{Y \mid X}} \]
\end{theorem}
\begin{proof}
    The proof follows directly from \cref{thm:iterated}. First use the definition of variance,
    \[ \Var\br{Y} = \Exp\br{Y^2} - \Exp\br{Y}^2 \]
    And then apply the law of iterated expectation on each term with respect to $X$,
    \[ \Var\br{Y} = \Exp\bc{\Exp\br{Y^2 \mid X}} - \Exp\bc{\Exp\br{Y \mid X}}^2 \]
    Now write $\Exp\br{Y^2 \mid X}$ in terms of an associated variance,
    \[ \Var\br{Y} = \Exp\bc{\Var\br{Y \mid X} + \Exp\br{Y \mid X}^2} - \Exp\bc{\Exp\br{Y \mid X}}^2 \]
    And let $\ga = \Exp\br{Y \mid X}$ for convenience,
    \[ \Var\br{Y} = \Exp\bc{\Var\br{Y \mid X} + \ga^2} - \Exp\bc{\ga}^2 \]
    Use linearity,
    \[ \Var\br{Y} = \Exp\bc{\Var\br{Y \mid X}} + \Exp\bc{\ga^2}- \Exp\bc{\ga}^2 \]
    And recognize a variance of $\ga$,
    \[ \Var\br{Y} = \Exp\bc{\Var\br{Y \mid X}} + \Var\bc{\ga} \]
    Replacing $\ga$ again,
    \[ \Var\br{Y} = \Exp\bc{\Var\br{Y \mid X}} + \Var\bc{\Exp\br{Y \mid X}} \]
\end{proof}
...
\begin{example}
    ...
    \begin{definition}
        We define the probability of extinction of a branching process to be $u$ where,
        \[ u = \lim_{n \to \inf} P\br{X_n = 0 \mid X_0 = 1} \]
    \end{definition}

    \begin{theorem}
        For a branching process, extinction happens with probability $u < 1$ if $\mu > 1$, where $u$ is the smallest solution between $0$ and $1$ of $\varphi\br{s} = s$ (where $\varphi$ is the generating function for $\xi$). Extinction happens with probability $1$ if $\mu \leq 1$. \\
    \end{theorem}

    \begin{proof}
        Note that,
        \[ \Exp\br{X_n} = \mu \Exp\br{X_{n-1}} \implies \Exp\br{X_n} = \mu^n \]
        Therefore,
        \[ \mu^n = \Exp\br{X_n} = \sum_{i = 1}^{\inf} i P\br{X_n = i} \geq \sum_{i = 1}^{\inf} P\br{X_n = i} = P\br{X_n \geq 1}\]
        Therefore for $\mu < 1$,
        \[ \lim_{n \to \inf} \mu^n = 0 = \lim_{n \to \inf} P\br{X_n \geq 1} = 1 - u \]
        Therefore $P\br{X_n = 0} = 1-P\br{X_n \geq 1} = u = 1$. The remaining cases are more difficult to demonstrate.
    \end{proof}
    ...
    \begin{definition}
        A \term{continuous time stochastic process} $\bc{X\br{t}}_{t \geq 0}$ is called a \term{continuous-time Markov chain} is its state space is at most countable and it satisfies the \term{continuous-time Markov property}.
    \end{definition}
    ...
    \begin{definition}
        Let $H_{ii}$ be the amount of (continuous) time until the MC revisits state $i$ given $X\br{0}= i$. A recurrent state $i$ is called positive recurrent if $\Exp\br{H_{ii}}< \inf$.
        \[\text{$i$ is positive recurrent: } \quad \Exp\br{H_{ii}}< \inf\]
        It is called null recurrent if $\Exp\br{H_{ii}} = \inf$.
        \[\text{$i$ is null recurrent: } \quad \Exp\br{H_{ii}} = \inf\]
    \end{definition}
    \begin{center}
        \begin{tikzpicture}
            \pgfmathsetmacro{\sa}{1};
            ...
            \draw[] (\sd, -\tick) -- (\sd, \tick);
            \draw[] (\se, -\tick) -- (\se, \tick);
            \draw[] (\sf, -\tick) node[below]{$H_{ii}$} -- (\sf, \tick);
    ...
    \begin{theorem}
        ...
        \[ \lim_{t \to \inf} P_{ij}\br{t} = \pi_{j} = \f{\Exp\br{T_j}}{\Exp\br{H_{jj}}} = \f{1 / \la_{j}}{\Exp\br{H_{jj}}} \quad \forall i, j \in S \]
    ...
    \end{theorem}
    Note that the last equality in \cref{thm:bltctmc} holds because $T_{j}$ is distributed exponentially with expectation $1 / \la_j$. Most of the terms and relations that hold for \cref{thm:bltctmc} are analogous to the discrete case.
    \[  T_j = \min\bc{t > 0 \mid X\br{t} \neq j, X\br{0} = j} \]
    \[  H_{jj} = \min\bc{t > T_j \mid X\br{t} = j} \]
    \begin{remark}
        The long-run fraction of time spent in state $j$ is given by $\Exp\br{T_j}/ \Exp\br{H_{jj}}$. The idea being that $H_{jj}$ represents the amount of time until the first return to state $j$ given that we started in state $j$ at time $0$.
        \begin{center}
            \begin{tikzpicture}
                \draw[|->] (0,0) node[below]{$0$} -- (5, 0) node[right]{$t$};
                \draw[red,thick,|-|] (0,0) -- node[below]{$T_j$} (2, 0);
                \draw[|->, red,thick] (4,0) node[below]{$H_{jj}$} -- (5, 0);
            \end{tikzpicture}
        \end{center}
        For each interval of length $H_{jj}$, $T_{j}$ is the portion of time spent in state $j$.
    \end{remark}
    ...
    \begin{example}
        \term{Positive recurrence for an m/m/s queue:}
    ...
    \end{example}
    \begin{example}
        \term{Positive recurrence for a population model:} Recall that for a population model we have that $\la_i = i \la + \al$ and $\mu_i = i \mu$. Therefore,
        \begin{align*}
        ...
        However if $\la = \mu$ and $\al < \la = \mu$ characterizing the converge is more complicated. As it turns out we can make use of the \term{Raabe-Duhamel's test}. The Raabe-Duhamel test determines if a generic series $\sum_n{a_n}$ is convergent or not. Let $L$ be,
    ...
    \begin{center}
        \begin{tikzpicture}
            \draw[<->] (0, -1) -- (0, 1) node[above]{$B_{\br{\De x, \De t}}\br{t}$};
            \draw[->] (0, 0) -- (5, 0) node[right]{$t$};
            \pgfmathsetmacro{\dt}{0.2};
            \pgfmathsetmacro{\dx}{0.2};
            \foreach \b [count = \i] in {1,2,1,0,-1,0,-1,-2,-3,-4,-3,-4,-5,-4,-3,-2,-3,-2,-1} {
                \draw[thick] (\dt*\i, \dx*\b) -- (\dt*\i + \dt*1, \dx*\b);
            }
        \end{tikzpicture}
    \end{center}
    ...
    \end{enumerate}
    \begin{definition}
        A continuous stochastic process $\bc{B\br{t}}_{t \geq 0}$ is called a \term{Brownian motion} if,
        \begin{enumerate}
            \item $B\br{0} = 0$
            \item $\bc{B\br{t}}_{t \geq 0}$ has stationary and independent increments
            \item $\forall t: B\br{t} \sim \s{N}\br{0, \si^2 t}$ for some $\si \geq 0$. If $\si = 1$ then we have \term{standard Brownian motion} (default setting working forward)
        \end{enumerate}
    \end{definition}
    \subsection{Properties of Brownian Motion}
    \begin{center}
    \begin{tikzpicture}
        \draw[thick, ->] (0, -3) -- (0, 3) node[above]{$B\br{t}$};
        \draw[thick, ->] (0, 0) -- (6, 0) node[right]{$t$};
        \Brownian{600}{0.01}{0.2}{red}{$B_1\br{t}$}
        \Brownian{600}{0.01}{0.2}{blue}{$B_2\br{t}$}
    \end{tikzpicture}
    \end{center}
    Brownian motion is a Gaussian process.
    \begin{definition}
        A group of r.v.s $\br{X_1, X_2, \ldots, X_n}$ are called \term{jointly normal} if $a_1X_1 + a_2 X_2 + \cdots + a_n X_n$ follows a normal distribution for any $a_1, \ldots, a_{n} \in \R$.
    \end{definition}
    \begin{definition}
        A stochastic process $\bc{X\br{t}}_{t \geq 0}$ is called a \term{Gaussian process} if all its finite dimensional distributions are jointly normal. That is $\forall n$, $t_1, \ldots, t_n$, $\br{X\br{t_1}, \ldots, X\br{t_n}}$ is jointly normal.
    ...
    \end{definition}
    The concept of a martingale comes from the idea of a \textit{fair game}: on average, the gamble does not win or lose; the expected future wealth is equal to the current wealth. We can now prove that Brownian motion is martingale.
    \begin{align*}
    ...
    \begin{definition}
        The \term{quadratic variation of $B$} denoted $\bs{B}$ is the defined as follows,
    ...
    \end{proof}
    \begin{theorem}
        \term{Lévy's characterization of BM:} Let $X = \bc{X\br{t}}_{t \geq 0}$ be martingale with $X\br{0} = 0$ and $\bs{X}\br{t} = t$, then $X$ is Brownian motion.
    \end{theorem}
    ...
    \begin{align*}
        ...
        \end{cases}
    \end{align*}
    Then the \term{reflection principle} claims that $B^{*}\br{t}$ is also a Brownian motion.
    \begin{definition}
    ...
    \begin{theorem}
        ...
        Therefore $M\br{t}$ has the same distribution as $\abs{B\br{t}}$ where $B\br{t} \sim \s{N}\br{0,1}$.
    \end{theorem}
    \begin{proof}
        First notice that,
    ...
    \begin{definition}
        A continuous process $\bc{X\br{t}}_{t \geq 0}$ is called a \term{Brownian motion with drift} $\mu$ and variance parameter $\si^2$ (volatility $\si$) if,
        \begin{enumerate}
            \item $X\br{0} = 0$
